...
// Define our sensors
SensorData sensors[] = {
  {25.5, 30.0, -999, 0, 60, "TEMP", "C", ORANGE, false, true},
  {3.28, 3.35, -999, 2.8, 3.6, "VOLT", "V", GREEN, false, true},
  {78.5, 85.0, -999, 0, 100, "CPU", "%", BLUE, false, true},
  {45.2, 50.0, -999, 0, 100, "MEM", "%", CYAN, false, true}
};
...
void loop() {
  ...
  // Initial full draw or refresh every 10 seconds to avoid artifacts
  if (!displayInitialized || (currentTime - lastUpdate > 10000)) {
    ...
    // Selective updates only - update individual sensors that changed
    for (int i = 0; i < numSensors; i++) {
      if (sensors[i].needsUpdate) {
        updateSensorDisplay(i);
        sensors[i].needsUpdate = false;
      }
    }
